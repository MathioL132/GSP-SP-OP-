...
struct sp_tree_node {
    ...
    sp_tree_node& operator=(sp_tree_node const& other) {
        if (this != &other) {
            delete l;
            delete r;
            source = other.source;
    ...
    sp_tree_node(sp_tree_node&& other) noexcept
        : source{other.source}, sink{other.sink}, comp{other.comp}, l{other.l}, r{other.r} {
        other.l = nullptr;
        other.r = nullptr;
    }
    
    sp_tree_node& operator=(sp_tree_node&& other) noexcept {
        if (this != &other) {
            delete l;
            delete r;
            source = other.source;
...
std::ostream& operator<<(std::ostream& os, sp_tree_node const& node) {
    ...
    os << "{" << node.source << ", " << node.sink << ", " << comp_names[static_cast<int>(node.comp)] << "}";
    return os;
}
...
struct sp_tree {
    ...
    sp_tree(int source, int sink) : root{new sp_tree_node{source, sink}} {}
    sp_tree(sp_tree const& other) : root{other.root ? new sp_tree_node{*other.root} : nullptr} {}
    sp_tree(sp_tree&& other) : root{other.root} { other.root = nullptr; }
    ...
    void compose(sp_tree&& other, c_type comp) {
        ...
        root = new sp_tree_node{root->source, root->sink, comp, root, other.root};
        other.root = nullptr;
    }
    
    void l_compose(sp_tree&& other, c_type comp) {
        compose(std::move(other), comp);
    }
...
struct positive_cert_gsp : certificate {
...
// ==================== SP RECOGNITION DATA STRUCTURES ====================
...
// ==================== CERTIFICATE AUTHENTICATION IMPLEMENTATIONS ====================
...
// Radix sort implementation for authentication
...
bool positive_cert_gsp::authenticate(graph const& g) {
    if (verified) return true;
...
std::vector<edge_t> get_bicomps(graph const&, std::vector<int>&, sp_recognition_result&, int = 0);
void report_K4_non_stack_pop_case(sp_recognition_result&, std::vector<int> const&, std::vector<std::stack<sp_chain_stack_entry>>&, int, int, int, int, int, int);
int path_contains_edge(std::vector<edge_t> const&, edge_t);

sp_recognition_result SP_RECOGNITION(graph const& g) {
    sp_recognition_result retval{};
    ...
    for (int bicomp = 0; bicomp < n_bicomps; bicomp++) {
        ...
        if (bicomp < n_bicomps - 1) {
            ...
            if (!retval.reason) {
                std::shared_ptr<positive_cert_gsp> gsp{new positive_cert_gsp{}};
...
std::vector<edge_t> get_bicomps(graph const& g, std::vector<int>& cut_verts, sp_recognition_result& cert_out, int root) {
    std::vector<int> dfs_no((size_t)(g.n), 0);
    std::vector<int> parent((size_t)(g.n), 0);
...
void report_K4_non_stack_pop_case(sp_recognition_result& cert_out,
    ...
    for (int b = parent[k4->b]; b != k4->d; b = parent[b]) {
        for (; !vertex_stacks[b].empty(); vertex_stacks[b].pop()) {
            if (vertex_stacks[b].top().end == k4->a) {
                earliest_violating_ear = std::move(vertex_stacks[b].top().SP);
                k4->c = b;
            }
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    sp_recognition_result result = SP_RECOGNITION(g);

    if (result.is_sp) {
        std::cout << "The graph is series-parallel.\n";
        std::shared_ptr<positive_cert_gsp> cert = std::dynamic_pointer_cast<positive_cert_gsp>(result.reason);
        if (cert) {
            std::cout << "SP decomposition tree root: {" << cert->decomposition.source() << ", " << cert->decomposition.sink() << "}\n";
        }
    } else {
        std::cout << "The graph is not series-parallel.\n";
        if (std::dynamic_pointer_cast<negative_cert_K4>(result.reason)) {
            std::cout << "Reason: K4 subdivision found\n";
        } else if (std::dynamic_pointer_cast<negative_cert_T4>(result.reason)) {
            std::cout << "Reason: Theta-4 subdivision found\n";
        } else if (std::dynamic_pointer_cast<negative_cert_tri_comp_cut>(result.reason)) {
            std::cout << "Reason: Cut vertex with 3+ components found\n";
        } else if (std::dynamic_pointer_cast<negative_cert_tri_cut_comp>(result.reason)) {
            std::cout << "Reason: Biconnected component with 3+ cut vertices found\n";
        }
    }
